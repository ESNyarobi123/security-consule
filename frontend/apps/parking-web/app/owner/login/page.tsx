...
import { ownerLogin } from '@pssms/api-client';
import { setOwnerSession } from '@pssms/auth';
import Link from 'next/link';
import { useRouter } from 'next/navigation';
import { FormEvent, useState } from 'react';

function canAccessOwnerPortal(user: {
  roles: string[];
  permissions: string[];
}): boolean {
  if (user.roles.includes('VEHICLE_OWNER')) return true;
  if (
...
export default function OwnerLoginPage() {
  ...
  const [email, setEmail] = useState('owner1@highlink.co.tz');
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!canAccessOwnerPortal(result.user)) {
        setError(
          'This account is not an approved vehicle owner (need VEHICLE_OWNER / parking.self).',
        );
        return;
      }
      setOwnerSession(
    ...
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col justify-center px-4 py-10">
      ...
      >
        ...
        <p className="mt-1 text-sm text-slate-500">
          View your vehicles, permits, and gate entries
        </p>
