...
import {
  getProviderMe,
  listProviderEntries,
  type ProviderVisitProfile,
  type VisitorEntry,
} from '@pssms/api-client';
import {
  clearProviderSession,
  getProviderSessionUser,
  getProviderToken,
} from '@pssms/auth';
...
export default function ProviderSelfViewPage() {
  const router = useRouter();
  const [me, setMe] = useState<ProviderVisitProfile | null>(null);
  ...
  const session = getProviderSessionUser();
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          <section>
            ...
            <BookVisitForm
              mode="self"
              token={getProviderToken() ?? undefined}
              defaultVisitKind="SUPPLIER_VISIT"
              defaultName={me?.fullName ?? ''}
              defaultEmail={me?.email ?? session?.email ?? ''}
              defaultCompany="TechCare Systems Ltd"
              onSuccess={(a) => {
