...
export default function FinanceInvoicesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (parkingOnly && (r.serviceType ?? '').toUpperCase() !== 'PARKING') {
        return false;
      }
      if (!q) return true;
  ...
  }, [rows, query, statusFilter, parkingOnly, customerName]);
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-[#1b1a19]">Customer invoices</h2>
          <p className="mt-0.5 max-w-2xl text-xs text-[#605e5c]">
            Issue, collect receipts, dispute, and close. Parking bills use service
            type PARKING. Alerts cover overdue, unpaid, payroll-due, and
            suspension risk.
          </p>
        </div>
