...
export default function PayrollCyclesPage() {
  ...
  const [tenantFilter, setTenantFilter] = useState<
    'all' | 'INTERNAL_COMPANY' | 'CUSTOMER_MANAGED_PAYROLL'
  >('all');
  ...
  }, [cycles, query, statusFilter, tenantFilter]);
  ...
  async function runReport(
    ...
    try {
      if (kind === 'register') {
        ...
        const rep = await getPayrollLoanReport(selected);
        const csv = [
          'employeeNumber,employeeName,loanCode,label,amount',
          ...rep.rows.map(
            (r) =>
              `${r.employeeNumber},${r.employeeName},${r.loanCode},${r.label},${r.amount}`,
          ),
        ].join('\n');
        const blob = new Blob([csv], { type: 'text/csv' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `${selectedCycle?.cycleCode ?? 'payroll'}-loans.csv`;
        a.click();
        URL.revokeObjectURL(url);
      } else if (kind === 'statutory') {
        const rep = await getPayrollStatutoryReport(selected);
        const csv = [
          'employeeNumber,employeeName,grossPay,nssf,paye',
          ...rep.rows.map(
            (r) =>
              `${r.employeeNumber},${r.employeeName},${r.grossPay},${r.nssf},${r.paye}`,
          ),
        ].join('\n');
        const blob = new Blob([csv], { type: 'text/csv' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `${selectedCycle?.cycleCode ?? 'payroll'}-statutory.csv`;
        a.click();
        URL.revokeObjectURL(url);
  ...
  function openCreate() {
    ...
    setCreateError(null);
    setShowCreate(true);
    void listPayrollCustomerOptions()
      .then(setCustomers)
      .catch(() => setCustomers([]));
  }
  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-[#1b1a19]">Payroll cycles</h2>
          <p className="mt-0.5 max-w-2xl text-xs text-[#605e5c]">
            Company and customer-managed cycles. Generate freezes snapshots
            (OT, allowances, alertness, loans, statutory). Approval is HR →
            Finance → GM → CEO — creator cannot approve. Pay only after APPROVED.
          </p>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between gap-3 rounded-lg border border-dashed border-[#e1dfdd] px-4 py-3 text-[13px] text-[#605e5c]">
          ...
          <button
            ...
            onClick={async () => {
              ...
              setError(null);
              try {
                await scanPayrollDueAlerts(false);
                await load();
              } catch (err) {
              ...
              <div className="flex flex-wrap gap-1">
                {(
                  [
                    ...
                    ['CUSTOMER_MANAGED_PAYROLL', 'Customer'],
                  ] as const
                ).map(([id, label]) => (
                  <button
                    key={id}
