...
const ACCESS_TOKEN_KEY = 'pssms.guard.accessToken';
const REFRESH_TOKEN_KEY = 'pssms.guard.refreshToken';
const USER_JSON_KEY = 'pssms.guard.user';
...
export async function getStoredUser(): Promise<GuardUser | null> {
  const raw = await SecureStore.getItemAsync(USER_JSON_KEY);
  if (!raw) return null;
  try {
    return JSON.parse(raw) as GuardUser;
  } catch {
    return null;
  }
}
